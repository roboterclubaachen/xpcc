 ...
 * 1) Sends out a SSTW-Ranging Frame on Broadcast
 * 2) Waits for 1000ms for an Answer and Computes the Range between the both
*/
...
main()
{
	...
	while (true)
	{
		...
		ranging::sendSSTWRinitAt(0xAAAA);
		timeout.restart(5);
		...
		if (isrx)
		{
			...
			XPCC_LOG_DEBUG << "Got Answer"<< xpcc::endl;
